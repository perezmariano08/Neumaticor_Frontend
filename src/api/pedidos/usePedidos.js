...
import { fetchPedido, fetchPedidoDetalle, fetchPedidoDetallePorUsuario, fetchPedidos, fetchPedidosPorUsuario } from './pedidos';
...
export const usePedidoDetalle = (id_pedido) => {
    const token = useSelector((state) => state.user.token);
    
    return useQuery({
        queryKey: ["pedido_detalle", id_pedido],
        queryFn: () => fetchPedidoDetalle(token, id_pedido),
        staleTime: 1000 * 60 * 0.5,
        enabled: !!token, // Solo hace la consulta si hay token
    });
};
